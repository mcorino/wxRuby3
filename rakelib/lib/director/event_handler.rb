      ...
      def setup
        ...
        if spec.module_name == 'wxEvtHandler'
          ...
          spec.map 'wxEvent *event' => 'Wx::Event' do
            add_header_code 'WXRUBY_EXPORT bool wxRuby_Is_RubyCommandEvent(wxEvent* wx_event);'
            ...
            # User defined command events however need to maintain their tracking register as we need to keep
            # the state of the linked Ruby objects. As WxW will clean up later we can however release ownership.
            map_in code: <<~__CODE
              $1 = (wxEvent*)DATA_PTR($input);
              if (!wxRuby_Is_RubyCommandEvent($1))
              {
