...
if [ "$_test" == "1" ]; then
  ...
  gem install ./$(echo wxruby3*.gem) -- package=`pwd`/$(echo wxruby3*.pkg) --no-format-executable && xvfb-run -a -s '-screen 0 1600x1200x24' wxruby test --exclude=test_intl --exclude=test_media_ctrl
