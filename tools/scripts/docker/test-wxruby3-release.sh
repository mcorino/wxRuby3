...
if [ "$_test" == "1" ]; then
  ...
  gem install ./$(echo wxruby3*.gem) --no-format-executable -- package=`pwd`/$(echo wxruby3*.pkg) && xvfb-run -a -s '-screen 0 1600x1200x24' wxruby test --keep-going --exclude=test_intl --exclude=test_media_ctrl
